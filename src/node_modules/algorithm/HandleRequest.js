...
function HandleRequestPart1(request)
{
	...
	for(var ii = 0 ; ii < possibleCutsitesTypes.length ; ++ii)
	{
		...
		request.UpdateState("Found " + rawCandidatesPerCutsite.length + " cutsite(s) for cutsite type " + cutsiteTypeCutsiteContainer.Type);
		numberOfCandidatesGenerated += rawCandidatesPerCutsite.length;
		...
		numberOfCandidatesGeneratedAndCleansed += rawCandidatesPerCutsite.length;
	...
	{
	    request.UpdateState("No candidates where generated!");
	    Log("No candidates where generated!", "HandleRequestPart1", 0);
